...
ClientManager::ClientManager(EventBus& n_event_bus, socket_t n_server_socket) : event_bus(n_event_bus), server_socket(n_server_socket) {
	std::thread t(&ClientManager::accept_new_client, this);
	t.detach();
	register_user_input_handler();
	register_send_command_to_client();
}
...
void ClientManager::accept_new_client() {
	...
	while (true) {
		...
		ClientHandler client_hander(client_id++, client_socket, std::string(client_address_ip), client_addr.sin_port, event_bus);
...
void ClientManager::register_user_input_handler() {
	...
	event_bus.subscribe([this, &event_bus_ref](const Event& event) {
		if (const InputEvent* input_event_ptr = dynamic_cast<const InputEvent*>(&event)) {
			...
			if (client_info.client_id <= 0) return;
			if (message.starts_with("config ")) {
				...
				try {
					...
					event_bus_ref.publish(NetworkSendEvent(client_info.client_ip, client_info.client_port, oss.str()));
...
void ClientManager::register_send_command_to_client() {
	event_bus.subscribe([this] (const Event& event) {
		if (const NetworkSendEvent* network_event_ptr = dynamic_cast<const NetworkSendEvent*>(&event)) {
			...
			ClientInfo client_info = this->get_client_info_from_ip_and_port(network_event.get_ip(), network_event.get_port());
			if (client_info.client_id == 0) return;
...
void ClientManager::send_command_to_client(int client_id, std::string command) {
...
ClientInfo ClientManager::get_client_info_from_ip_and_port(const std::string& client_ip, std::uint16_t client_port) {
	std::lock_guard<std::mutex> lock(client_handlers_mutex);
	for (const auto& [id, client] : client_handlers) {
		if (client.get_client_info().client_ip != client_ip) continue;
		if (client.get_client_info().client_port != client_port) continue;
